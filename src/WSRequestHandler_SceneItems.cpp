...
*
* @param {String (optional)} `scene` the name of the scene that the source item
...
* @return {int} `item.position.alignment` The point on the source that the item
* is manipulated from.
* @return {double} `item.rotation` The clockwise rotation of the item in
...
* @return {int} `item.crop.top` The number of pixels cropped off the top of the
* source before scaling.
* @return {int} `item.crop.right` The number of pixels cropped off the right of
* the source before scaling.
* @return {int} `item.crop.bottom` The number of pixels cropped off the bottom
* of the source before scaling.
* @return {int} `item.crop.left` The number of pixels cropped off the left of
* the source before scaling.
		...
		WSRequestHandler *req)
{
	...
	obs_data_set_string(resItem, "name", obs_source_get_name(
	...
	obs_data_set_int(posData, "alignment",
			obs_sceneitem_get_alignment(sceneItem));
	obs_data_set_obj(resItem, "position", posData);

	obs_data_set_double(resItem, "rotation",
			obs_sceneitem_get_rot(sceneItem));
	...
	case OBS_BOUNDS_SCALE_INNER:
		obs_data_set_string(boundsData, "type",
	...
	if (boundsType != OBS_BOUNDS_NONE) {
		obs_data_set_int(boundsData, "alignment",
	...
	return req->SendOKResponse(resData);
}
...
* @param {double} `item.rotation` The new clockwise rotation of the item in
* degrees.
* @param {double} `item.scale.x` The new x scale of the item.
* @param {double} `item.scale.y` The new y scale of the item.
* @param {int} `item.crop.top` The new amount of pixels cropped off the top of
* the source before scaling.
* @param {int} `item.crop.bottom` The new amount of pixels cropped off the
* bottom of the source before scaling.
* @param {int} `item.crop.left` The new amount of pixels cropped off the left
* of the source before scaling.
* @param {int} `item.crop.right` The new amount of pixels cropped off the right
* of the source before scaling.
* @param {bool} `item.visible` The new visibility of the item. 'true' shows
* source, 'false' hides source.
* @param {bool} `item.locked` The new locked of the item. 'true' is locked,
* 'false' is unlocked.
* @param {String} `item.bounds.type` The new bounds type of the item.
* @param {int} `item.bounds.alignment` The new alignment of the bounding box.
* (0-2, 4-6, 8-10)
		...
		WSRequestHandler *req)
{
	...
	if (obs_data_has_user_value(reqItem, "position")) {
		...
		OBSDataAutoRelease reqPosition = obs_data_get_obj(reqItem,
				"position");
		...
		if (obs_data_has_user_value(reqPosition, "alignment")) {
			auto alignment = (uint32_t)obs_data_get_int(reqPosition,
					"alignment");
			if (Utils::IsValidAlignment(alignment)) {
				obs_sceneitem_set_alignment(sceneItem,
						alignment);
			} else {
				badRequest = true;
				obs_data_set_string(positionError, "alignment",
						"invalid");
				obs_data_set_obj(errorMessage, "position",
						positionError);
			}
		}
	...
	if (obs_data_has_user_value(reqItem, "rotation"))
		obs_sceneitem_set_rot(sceneItem,
				(float)obs_data_get_double(reqItem,
						"rotation"));
	...
	if (obs_data_has_user_value(reqItem, "scale")) {
		...
		OBSDataAutoRelease reqScale = obs_data_get_obj(reqItem,
				"scale");
	...
	if (obs_data_has_user_value(reqItem, "crop")) {
		...
		if (obs_data_has_user_value(reqCrop, "bottom"))
			newCrop.bottom = (int)obs_data_get_int(reqCrop,
					"bottom");
	...
	if (obs_data_has_user_value(reqItem, "visible"))
		obs_sceneitem_set_visible(sceneItem,
				obs_data_get_bool(reqItem, "visible"));

	if (obs_data_has_user_value(reqItem, "locked"))
		obs_sceneitem_set_locked(sceneItem,
				obs_data_get_bool(reqItem, "locked"));

	if (obs_data_has_user_value(reqItem, "bounds")) {
		bool badBounds = false;
		OBSDataAutoRelease boundsError = obs_data_create();
		OBSDataAutoRelease reqBounds = obs_data_get_obj(reqItem,
				"bounds");
		if (obs_data_has_user_value(reqBounds, "type")) {
			string newBoundsType = obs_data_get_string(reqBounds,
					"type");
			if (newBoundsType == "OBS_BOUNDS_NONE") {
				...
				obs_data_set_string(boundsError, "type",
						"invalid");
			}
		}
		vec2 oldBounds = {};
		...
		if (obs_data_has_user_value(reqBounds, "x"))
			newBounds.x = (float)obs_data_get_double(reqBounds,
					"x");
		if (obs_data_has_user_value(reqBounds, "y"))
			newBounds.y = (float)obs_data_get_double(reqBounds,
					"y");
		obs_sceneitem_set_bounds(sceneItem, &newBounds);
		if (obs_data_has_user_value(reqBounds, "alignment")) {
			auto bounds_alignment = (uint32_t)obs_data_get_int(
					reqBounds, "alignment");
			if (Utils::IsValidAlignment(bounds_alignment)) {
				obs_sceneitem_set_bounds_alignment(sceneItem,
						bounds_alignment);
			} else {
				badRequest = badBounds = true;
				obs_data_set_string(boundsError, "alignment",
						"invalid");
			}
		}
		if (badBounds)
	...
	if (badRequest)
		return req->SendErrorResponse("bad scene item params",
				errorMessage);
...
* Reset a scene item.
*
* @param {String (optional)} `scene-name` Name of the scene the source belogns
* to. Defaults to the current scene.
...
OBSDataAutoRelease WSRequestHandler::HandleResetSceneItem(WSRequestHandler *req)
{
	...
	string itemName = obs_data_get_string(req->data, "item");
	if (itemName.empty())
		return req->SendErrorResponse("invalid request parameters");
...
* @param {String (optional)} `scene-name` Name of the scene where the source
* resides. Defaults to the currently active scene.
*
* @api requests
...
OBSDataAutoRelease WSRequestHandler::HandleSetSceneItemRender(
		WSRequestHandler *req)
{
	if (!req->hasField("source") || !req->hasField("render"))
		return req->SendErrorResponse("missing request parameters");
...
*
* @param {String (optional)} `scene-name` The name of the scene that the source
* item belongs to. Defaults to the current scene.
...
OBSDataAutoRelease WSRequestHandler::HandleSetSceneItemPosition(
		WSRequestHandler *req)
{
	if (!req->hasField("item") || !req->hasField("x") ||
			!req->hasField("y"))
		return req->SendErrorResponse("missing request parameters");
	...
	if (!scene)
		return req->SendErrorResponse(
				"requested scene could not be found");
	...
	vec2 item_position = {};
	item_position.x = (float)obs_data_get_double(req->data, "x");
	item_position.y = (float)obs_data_get_double(req->data, "y");
...
OBSDataAutoRelease WSRequestHandler::HandleSetSceneItemTransform(
		WSRequestHandler *req)
{
	if (!req->hasField("item") || !req->hasField("x-scale") ||
			!req->hasField("y-scale") || !req->hasField("rotation"))
		return req->SendErrorResponse("missing request parameters");
	...
	vec2 scale = {};
	scale.x = (float)obs_data_get_double(req->data, "x-scale");
	scale.y = (float)obs_data_get_double(req->data, "y-scale");
	obs_sceneitem_set_scale(sceneItem, &scale);
	obs_sceneitem_set_rot(sceneItem,
			(float)obs_data_get_double(req->data, "rotation"));
		...
		WSRequestHandler *req)
{
	...
	crop.top    = (int)obs_data_get_int(req->data, "top");
	crop.bottom = (int)obs_data_get_int(req->data, "bottom");
	crop.left   = (int)obs_data_get_int(req->data, "left");
	crop.right  = (int)obs_data_get_int(req->data, "right");
